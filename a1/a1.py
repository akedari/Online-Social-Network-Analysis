...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    wt_dict = {}
    final = {}
    distances =  sorted(node2distances.items(), key=lambda x: x[1], reverse=True)

    for node in distances:
        wt_dict[node[0]] = 1

    for node in distances:
        if not node2distances[node[0]] == 0:
            current = node[0]
            temp_parent = node2parents[current]

            for par in temp_parent:
                parent = par
                wt_dict[parent] = wt_dict[parent] + (wt_dict[current] / node2num_paths[current])
                #final[(current, parent)] = wt_dict[current] / node2num_paths[current]
                if current<parent:
                    final[(current, parent)] = wt_dict[current]/ node2num_paths[current]
                else:
                    final[(parent,current)] = wt_dict[current] / node2num_paths[current]
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in nodes:
        ...
        results = bottom_up(node,node2distances,node2num_paths,node2parents)

        for key, value in results.items():
            if key in betweewnness:
                betweewnness[key] = betweewnness[key]+ (value/2)
            else:
                betweewnness[key] = value/2
    return betweewnness
...
def partition_girvan_newman(graph, max_depth):
    ...
    sorted_betweennness = sorted(betweenness.items(), key=lambda x: (-x[1],x[0]))
    components = list(nx.connected_component_subgraphs(graph_copy))
    while len(components) == 1:
        element = sorted_betweennness.pop(0)
        graph_copy.remove_edge(element[0][0], element[0][1])
        components = list(nx.connected_component_subgraphs(graph_copy))
    return components
...
def make_training_graph(graph, test_node, n):
    ...
    #sorted_nodes = sorted(copyOfGraph.neighbors(test_node))
    nodes = copyOfGraph.neighbors(test_node)
    sorted_nodes = sorted(nodes)

    for i in range(0,n):
        copyOfGraph.remove_edge(test_node,sorted_nodes[i])
...
def path_score(graph, root, k, beta):
    ...
    result = bfs(graph,root,float("inf"))
    node2distances = result[0]
    node2num_paths = result[1]

    for node in graph_copy.node:
        if not root == node:
            if not graph.has_edge(root,node):
                value = beta ** node2distances[node] * node2num_paths[node]
                pathscore[(root,node)]=value

    return sorted(pathscore.items(),key=lambda x:(-x[1],x[0]))[:k]
...
def evaluate(predicted_edges, graph):
    ...
    """
    ###TODO
    """set1 = set(el[0] for el in graph.edges())
    set2 = set(el[0] for el in predicted_edges)
    set3 = set1 & set2
    result = (len(set3) / (len(predicted_edges)))
    return  result"""
    ...
    if edge in predicted_edges:
        count = count + 1
    return float(count / len(predicted_edges))"""
