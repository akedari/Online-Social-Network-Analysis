...
def read_screen_names(filename):
    ...
    ###TODO
    cadidatenames =[]
    cadidates = open("candidates.txt", 'r')
    cadidatenames = [name.strip() for name in cadidates.readlines()]
    cadidates.close()
    return cadidatenames
...
def friend_overlap(users):
    ...
    while(i<n-1):
        j=i+1
        while(j<n):
            c = Counter(users[i]['friends'])
            d = Counter(users[j]['friends'])
            unique = c&d
            tup=(users[i]['screen_name'],users[j]['screen_name'],len(unique))
            result.append(tup)
            j=j+1
        i=i+1
    sorted_users1 = sorted(result, key=lambda x: (x[0],x[1]))
    sorted_users = sorted(sorted_users1, key=lambda x: x[2], reverse=True)
    return  sorted_users
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for user in users:
        if(user['screen_name']=="HillaryClinton"):
            c = Counter(user['friends'])
        if(user['screen_name']=="realDonaldTrump"):
            d = Counter(user['friends'])
    unique = c & d
    for r in unique:
            commonuserId = r

    response = robust_request(twitter,'users/lookup', {'user_id': commonuserId})
    cm_user = [user for user in response]
    commonUser = cm_user[0]['screen_name']
    return commonUser
...
def create_graph(users, friend_counts):
    ...
    while (i < n - 1):
        ...
        while (j < n):
            ...
            #tup = (users[i]['screen_name'], users[j]['screen_name'], len(unique))
            #result.append(tup)
            for othernode in unique:
                nodes.append(othernode)
                edge_tup1=(users[i]['screen_name'],     othernode)
                edge_tup2 = (users[j]['screen_name'], othernode)
                edges.append(edge_tup1)
                edges.append(edge_tup2)
            j = j + 1
        i = i + 1

    #print("noumber of nodes %d and edges are %d" %(len(nodes),len(edges)))
